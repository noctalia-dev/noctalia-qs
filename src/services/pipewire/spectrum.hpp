...
/// producing bar values suitable for audio visualization.
///
...
class PwAudioSpectrum: public QObject {
	...
	/// Number of frequency bars to produce. Defaults to 32.
	...
	/// Enable smoothing between adjacent bars for a less jittery look. Defaults to true.
	Q_PROPERTY(bool smoothing READ smoothing WRITE setSmoothing NOTIFY smoothingChanged);
	/// Per-bar spectrum values, normalized to 0.0-1.0. Length equals @@barCount.
	Q_PROPERTY(QList<float> values READ values NOTIFY valuesChanged);
	/// True when audio is silent (all bars near zero) for several consecutive frames.
